...
bool PointLiesOnLine(const Point & p, const Point & q, const Point & r)
{
	//To check if q is on line pr
...
float areaTriangle(const Point & p1,const Point & p2, const Point & p3)
{
...
bool intersectsLine(const Point & line1, const Point & line2, const Point & p, const Point & q)
{
	// Find the four orientations needed for general and
	// special cases
	int o1 = orientation(line1, line2, p);
	int o2 = orientation(line1, line2, q);
	int o3 = orientation(p, q, line1);
	int o4 = orientation(p, q, line2);
	...
	// Special Cases
	// line1, line2 and p are colinear and p lies on segment line1line2
	if (o1==0 && PointLiesOnLine(line1, p, line2))
	{
		return true;
	}

	// line1, line2 and p are colinear and q lies on segment line1line2
	if (o2==0 && PointLiesOnLine(line1, q, line2))
	{
		return true;
	}

	// p, q and line1 are colinear and line1 lies on segment pq
	if (o3==0 && PointLiesOnLine(p, line1, q))
	{
		return true;
	}

	// p, q and line2 are colinear and line2 lies on segment pq
	if (o4==0 && PointLiesOnLine(p, line2, q))
	{
		return true;
	}
...
bool PolygonS::contains(Point q)
{
	...
	if (n==2)
	{
		return PointLiesOnLine(p[0],q,p[1]);
	}
	Point extreme;
	extreme.x=1000;
	extreme.y=q.y;
	...
	for (int i=0,j=i+1;i<n;i++)
	{
		...
		if (intersectsLine(p[i],p[j],q,extreme))
		{
			if (orientation(p[i],q,p[j]) == 0)
			{
				return PointLiesOnLine(p[i],q,p[j]);	//if q lies on any of the line segments forming the polygon
			}
			count++;
		}
		else if (p[i].x-q.x>=-1 && p[i].x-q.x<=1 && p[i].y-q.y>=-1 && p[i].y-q.y<=1)
		{
			return true;
		}
		j++;
...
bool Triangle::contains(Point q)
{
	...
	for (int i=0;i<n-1;i++)
	{
		temp=areaTriangle(p[i],p[i+1],q);
		area.push_back(temp);
	}
	temp=areaTriangle(p[2],p[0],q);
	area.push_back(temp);
	temp=areaTriangle(p[0],p[1],p[2]);
	if (temp==area[0]+area[1]+area[2])
	{
		return true;
	}
	else
...
void RectangleS::fill(int color)
{
	...
	{
		GP142_lineXY(color,x1+1,j,x2-1,j,2);
	}
}
...
bool Text::contains(Point test)
{
	...
	if (test.y>=p[0].y && test.y<=(p[0].y+20))
	{
		if (x==true)
		{
			return true;
