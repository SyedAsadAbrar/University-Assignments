...
;brick printing;
brick: db '          '
brick_row: dw 0x03, 0x05
brick_col: dw 0x03, 0x10, 0x1D, 0x2A, 0x37, 0x44
brick_end: dw 0x0D, 0x1A, 0x27, 0x34, 0x41, 0x4D
row_found: dw 0
col_found: dw 0
brick_count: db 6
brick_lines: db 2
...
border_count: db 2
bonus: dw 100
timetext: db 'Time elapsed:     seconds'
...
kbisr:
	...
	sub byte[bar_col], 1
	cmp byte[bar_col], 0x00
	jne no_lfix
	mov byte[bar_col], 0x45
	no_lfix:
	call print_bar
	...
	nextcmp:
		...
		mov byte[bar_color], 0x30
		add byte[bar_col], 1
		cmp byte[bar_col], 0x46
		jne no_rfix
		mov byte[bar_col], 0x01
		no_rfix:
		call print_bar
