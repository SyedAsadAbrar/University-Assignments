...
--Q1
select Name,count(CourseID) as "Number of Subjects" 
from Students,Registration
where RollNo=RollNumber
group by Name

--Q2
select min(GPA) as "Minimum GPA", max(GPA) as "Maximum GPA", AVG(GPA) as "Average GPA", STDEV(GPA) as "Standard Deviation of GPA"
from Registration 

--Q3
select top 1 Name, sum(C.CreditHours) as Cred_Hours
from Registration as R join Courses as C on R.CourseID=C.CourseID join Students as S on S.RollNo=R.RollNumber
group by Name
order by Cred_Hours desc

--Q4

select top 2 Name, sum(C.CreditHours) as Cred_Hours
from Registration as R join Courses as C on R.CourseID=C.CourseID join Students as S on S.RollNo=R.RollNumber
group by Name
...
from Instructors
where Instructors.InstructorID=(Select distinct A.InstructorID
from Courses_Semester A join Courses_Semester B on A.InstructorID=B.InstructorID
where A.Section!=B.Section)

--Q6
select count(Semester), Semester, CourseID
...
select S.Name
from Students S
where exists(select * from Students So where So.WarningCount=0 AND So.Name=S.Name)

--Q9
select count(Name) as "Names not starting from A"
from Students
where Name NOT like 'A%'

--Q10
select Semester,sum(TotalDues) as "Dues submitted per semester"
from ChallanForm
where Status='Paid'
group by Semester