...
class Book(models.Model):
	...
	def getBooks(t):
		...
		booksTable = {}
		with connections['mysql'].cursor() as cursor:
			#cursor.execute(" select id,title,isbn from books where title like '%" + t + "%'")
	...
	def getBooksFromGoogle(t):
		books = []
		t = t.replace(' ','+')
		with urllib.request.urlopen('https://www.googleapis.com/books/v1/volumes?q='+t) as response:
			obj = json.loads(response.read())
			
			for x in obj['items']:
				book = Book(x['id'],x['volumeInfo']['title'],'')
				books.append(book)
