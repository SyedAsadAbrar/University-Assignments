...
public class Librarian extends User{
    ...
    Librarian(String username, String password, String name, String address, int phone)
    {
...
//    }
    ...
    //Books related tasks
    boolean addBook(Book b) throws SQLException
    {
        boolean status=false;
        String name=b.name;
        String author=b.author;
        String isbn=b.isbn;
        String subjetc=b.subject;
        int qty=b.quantity;
        Connection con = null;
        Statement stmt = null;
        ResultSet rs = null;    
        ...
        try
        {
            while(rs.next())
            {
                if(rs.getString(1).equals(b.name) && rs.getString(2).equals(b.author) && rs.getString(3).equals(b.isbn) && rs.getString(4).equals(b.subject) && rs.getString(5).equals(b.quantity))
                {
                    pStmt= = con.prepareStatement("update Books set quantity=quantity+1 where isbn=?");
                    rs=pStmt.executeQuery();
                    status=true;
                    return status;
                }
    ...
    boolean removeBook(Book b) throws SQLException
    {
        boolean status=false;
        String name=b.name;
        String author=b.author;
        ...
        try
        {
            pStmt = con.prepareStatement("remove from Books where isbn=rs.getString(2)");
            status=true;
        }
        catch(SQLException e)
        {
...
//            pStmt.setString(3, subject);
    ...
    void DisplayScreen() throws SQLException
    {
        ...
        System.out.println("Press 'a' for adding a new item.");
        System.out.println("Press 'd' for deleting an item.");
        System.out.println("Press 'c' for changing item info.");
        ...
        if(search == "1")
        {
            ...
            System.out.println("Press 3 for Searching Books by ISBN.");
